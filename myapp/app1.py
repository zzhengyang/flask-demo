...
def index():
    save(student)
    return render_template('students/../templates/index.html', student=student)   #这里的student=student，等号左边是要传给html文件的变量，右边是上面定义的二维列表
...
def delstu():
    ...
    return render_template('students/../templates/delstu.html', student=student)

@app.route('/add',methods=['GET','POST'])               #这里是添加模块
def addstu():
    if request.method=='POST':
    ...
    return render_template('students/../templates/addstu.html')                     #添加成功则重定向到首页
        ...
        if findnumber(altnumber)==-1:
            return '您输入的学号不存在'
                ...
                if altname!='':
                    ...
                    save(student)
                return render_template('students/../templates/index.html', student=student)


    return render_template('students/../templates/altstu.html')
...
def searchstu():
    if request.method=='POST':
        ...
        if findnumber(number)==-1:
            ...
            return render_template('students/../templates/searchstu.html', find=find)      #如果找到了学号，把该学生信息传给html
    return render_template('students/../templates/searchstu.html', student=student)
